...
if(not os.path.exists("../raw/insample")): 
    os.makedirs("../raw/insample", exist_ok=True)
if(not os.path.exists("../raw/outsample")): 
    os.makedirs("../raw/outsample", exist_ok=True)

# import os 
# print(os.listdir())
indexStocksDf = pd.read_csv('../raw/csi500_index_composition.csv', index_col=None, encoding='gbk') 
...
for stock in tqdm(indexStocksDf.code): 
    ...
    result.to_csv(f"../raw/insample/history_stock_{stock_ticker}.csv", index=False)
    ...
    result = pd.DataFrame(data_list, columns=rs.fields) 
    result.rename(columns = str.capitalize, inplace=True) 
    result.to_csv(f"../raw/outsample/history_stock_{stock_ticker}.csv", index=False)
